...
Model Project Euler's problem 345 (the maximum matrix sum) as a mathematical program, with the matrix loaded from `data1.dat`.

The statement itself is just a link to projecteuler.net/problem=345 and an image (`assets/euler345.jpg`) that was not included in the download.
...
- `handout/24-matrix-sum-euler-345.md` — the original Caseine statement, wording untouched
- `euler.mod` — blank OPL template: data declarations given, objective and constraints empty
- `data1.dat` — the matrix for the instance
- `configExe` — the Caseine grader config, names `euler.mod` and `data1.dat`

# utils.R -- Fonctions et configuration partagées (bundle Moodle autonome)
# Tous les chemins sont relatifs au dossier courant (moodle/).
# Pour ouvrir dans RStudio : Session → Set Working Directory → To Source File Location,
# ou bien ouvrir le .Rproj s'il existe. Le render du .Rmd règle ce chemin automatiquement.
...
# ── Chemin du cache modèles (relatif au .Rmd) ────────────────────────────────
OUTPUT_DIR <- "output"
...
# ── Chargement train/test/feature_cols ───────────────────────────────────────
load_train_test <- function() {
  list(
    train_data   = readRDS("output/train_data.rds"),

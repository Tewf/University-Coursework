# Worked example from the short OPL tutorial

Save, run and evaluate the example model exactly as given, to confirm the Caseine VPL workflow works end to end, without changing anything in it.
...
- `handout/01-opl-example.md` — the original Caseine statement, wording untouched
- `example.mod` — the tutorial's own worked example: a complete three-variable model, ready to run
- `configExe` — the Caseine grader config, names `example.mod`

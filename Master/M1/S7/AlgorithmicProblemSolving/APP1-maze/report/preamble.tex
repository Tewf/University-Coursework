...
% Short paragraphs read badly with first-line indents: space them instead.
\usepackage{parskip}
\usepackage{geometry}
...
% --- Emphasis ------------------------------------------------------------
% Three levels and no more, so that emphasis keeps meaning something:
%   \keyterm  a term at the point it is defined, or the load-bearing word of a
%             sentence -- bold, dark, and never more than one per paragraph;
%   \doorword / \wallword  the two words the figures are colour-coded for, so
%             prose and picture agree; used at definitions, not on every mention;
%   keypoint  a boxed claim the rest of the section leans on. Four in the report.
\definecolor{keyink}{HTML}{263238}
\newcommand{\keyterm}[1]{\textbf{\textcolor{keyink}{#1}}}
\newcommand{\doorword}[1]{\textbf{\textcolor{doorgreen}{#1}}}
\newcommand{\wallword}[1]{\textbf{\textcolor{wallred}{#1}}}

\newtcolorbox{keypoint}{%
  enhanced, breakable, colback=linkblue!4, colframe=linkblue!65,
...
\newcommand{\maze}{M}
\newcommand{\cells}{V}
\newcommand{\region}[2]{(#1)\text{--}(#2)}  % a region: two opposite corners

...
% drawn on the left is one internal node on the right, and the parameters
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{Cuts and tree nodes are in bijection. An internal node records the
  orientation $t$, the wall position $j$ and the door position $i$ that made
  the cut, and its two children are the two regions the cut created; a leaf is
  a single cell. Carrying this $4\times4$ example down to its leaves gives $16$
  leaves and $15$ internal nodes --- one per door, matching the $WH-1$ edges of

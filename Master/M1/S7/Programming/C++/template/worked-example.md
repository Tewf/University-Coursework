...
A project that uses every part of the `cpp-python` template: a graph library, a
shortest-path algorithm on it, two programs, tests, and a Python module. Every
command and output below was run on this machine on 2026-09-14.

```
shortest-path/                         ./new-project.sh cpp-python ../shortest-path
├── include/graph.hpp                  class Graph: vertices, add_edge, neighbours
├── include/shortest_path.hpp          shortest_path(graph, source, target)
├── src/graph.cpp                      the library: what both programs, the tests
├── src/shortest_path.cpp                and the module share, and nothing else
...
- **`bindings/graphs.cpp` is glue only.** `py::class_<Graph>` and `m.def` name
  the C++ entry points; the module is called `graphs` because the file is.
...
| Time it | `cmake --preset release && cmake --build --preset release`, then `./build/release/benchmark 300 4` | `4 queries on 90000 vertices: 0.0031 s` |
| Same in debug | `./build/debug/benchmark 300 4` | `0.0123 s`: four times slower, which is why timings are taken in `release` |
| Memory check | `cmake --preset asan && cmake --build --preset asan && ctest --preset asan` | `100% tests passed`, no report |
| Python | `conda activate m1ai-programming`, `cmake --preset python && cmake --build --preset python` | `Built target graphs`, a `.so` in `build/python/` |
| Import | from `build/python/`: `import graphs; g = graphs.Graph(5); ...` | `shortest_path(g, 1, 3)` gives `[1, 0, 3]`; `g.add_edge(0, 9)` raises `IndexError` |

The four build directories coexist, so switching from a Debug session under
...
`std::thread` linked without a flag, the fetched or system Google Test and the
env's pybind11 were found, and `-Wall -Wextra -Wpedantic -Werror` passed on all
nine source files at the first build.

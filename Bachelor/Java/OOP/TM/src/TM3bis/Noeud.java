...
public class Noeud extends NoeudAbstrait {
    ...
    public Character getNextChar(String s) throws FinDeTexteInattendueException {
        if (s == null || s.isEmpty()) {
            throw new FinDeTexteInattendueException("Texte codé terminé avant d’atteindre une feuille.");
        }
        char bit = s.charAt(0);
        if (bit == '0') {
    ...
    public void fournitPoids(Map<Character, Integer> m) {
        // Collecte les poids des enfants gauche et droit
        gauche.fournitPoids(m);
        droit.fournitPoids(m);
    }
}